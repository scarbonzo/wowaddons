...
If the animation's duration is set to |cFF00CC0010%|r, and the display's trigger is a buff that has no set duration, no start animation will play (although it would if you specified a duration in seconds).]=]
	...
	L["Can be a name or a UID (e.g., party1). Only works on friendly players in your group."] = "可以是名字或是 UID (例如 party1)，名字只對隊伍中的友方玩家有效。"
	L["Cancel"] = "取消"
...
是否要將這個顯示效果放到一個新的 |cFF22AA22動態群組|r 裡面?]=]
	...
	L["Convert to..."] = "轉換成..."
	L["Cooldown"] = "冷卻"
	L["Copy settings from %s"] = "從 %s 複製設定"
	L["Copy settings from..."] = "複製設定..."
...
UNIT_POWER, UNIT_AURA PLAYER_TARGET_CHANGED]=]
	...
	L["Do not copy any settings"] = "不要複製任何設定"
...
只有符合這一個值的時候會被載入]=]
	...
	L["Preset"] = "預設配置"
	L["Prevents duration information from decreasing when an aura refreshes. May cause problems if used with multiple auras with different durations."] = "光環更新時防止持續時間減少的資訊，使用不同持續時間的多個光環時可能會造成問題。"
	...
	L["Spell ID"] = "法術 ID"
	L["Spell ID dialog"] = [=[你使用了 |cFFFF0000法術 ID|r 來指定光環。

預設的情況下，|cFF8800FFWeakAuras|r 不會分辨名稱相同的光環，但是 |cFFFF0000spell ID| 不一樣。
然而，啟用了完整掃描的選項之後，|cFF8800FFWeakAuras|r 可以掃描指定的 |cFFFF0000spell ID|r。

是否要啟用完整掃描來匹配這個 |cFFFF0000spell ID|r?]=]
	L["Stack Count"] = "堆疊數量"
	L["Stack Info"] = "堆疊訊息"
	L["Stacks"] = "堆疊層數"
	...
	L["Use tooltip \"size\" instead of stacks"] = "使用滑鼠提示的 \"大小\" 取代堆疊"
